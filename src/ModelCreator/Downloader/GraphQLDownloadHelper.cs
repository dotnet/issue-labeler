...
class GraphQLDownloadHelper
{
        ...
        StreamWriter outputWriter)
    {
        try
        {
            ...
            {
                using (var client = GitHubHelpers.ClientConnection.CreateGraphQLClient())
                {
    ...
    private static void WriteCsvIssue(Dictionary<(DateTimeOffset, long, string), string> outputLines, IssuesNode issue, IssueType issueType
        ...
        , string repo)
    {
        var author = issue.Author != null ? issue.Author.Login : GitHubHelpers.GitHubUser.DeletedUser;
