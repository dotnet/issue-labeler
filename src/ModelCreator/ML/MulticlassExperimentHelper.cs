...
public static class MulticlassExperimentHelper
{
        ...
        MulticlassExperimentProgressHandler progressHandler, IDataView dataView, ColumnInferenceResults columnInference)
    {
        ...
        // Pre-featurize the title and description, and remove features that have less than 2.
    ...
    public static void TestPrediction(MLContext mlContext, DataFilePaths files, bool forPrs, double threshold = 0.4)
    {
        ...
        if (forPrs)
        {
            ...
            var prEngine = mlContext.Model.CreatePredictionEngine<GitHubPullRequest, AreaPrediction>(trainedModel);
        ...
        else
        {
            ...
            var issueEngine = mlContext.Model.CreatePredictionEngine<GitHubIssue, AreaPrediction>(trainedModel);
            predictions = testData
               .Select(x => (
                    knownLabel: x.Area,
